...
void VisualOdometryStereo::filterValues(std::vector<Matcher::p_match> p_matched)
{
  double x,y,z;
  ...
  for (int32_t i=N-1; i>=0; i--) {
    ...
    // filtering z values > 20.0 meters (1 pixel disparity) results in worse results in vrc_task_1
    // z values of < 2.0 meters are assumed to be part of the car
    // could also filter based on pixel values (v1p/u1p/v2p/u2p)
    if (z < 2.0) {
	p_matched.erase(p_matched.begin()+i);
	//printf("erase %lf %lf %lf %lf %lf %d\n", x,y,z, p_matched[i].u1p, p_matched[i].v1p, i);
    }
    else {
	//printf("filter %lf %lf %lf %lf %lf\n", x,y,z, p_matched[i].u1p, p_matched[i].v1p);
    }
  }
}
